...
const getUploadFile = async (req, res) => {
  try {
    const uploadFiles = req.files; // Assume req.files contains uploaded file data
    const { from, to } = req.body;
    ...
    const filePath = path.join(uploadDir, uploadFiles.files.name);

    // Save the file locally
    fs.writeFileSync(filePath, uploadFiles.files.data);

    // Create the URL to access the file
    const fileUrl = `/uploads/${uploadFiles.files.name}`;

    // Save file URL in the message (similar to how you did with S3)
